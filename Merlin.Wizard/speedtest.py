...
def speedtest(list = False , mini = None , server = None , share = True , simple = False , src = None , timeout = 10 , units = ( 'bit' , 8 ) , version = False):
	...
	dp.create("[COLOR aqua]Speed Test[/COLOR]", "[COLOR yellow]Starting Test..[/COLOR]", "", "")
	dp.update(0)
	...
	print_('Retrieving Server List...')
	dp.update(4, "", "", "[COLOR aqua]Retrieving Server List...[/COLOR]")

	servers = closestServers(config['client'])

	print_('Testing from %(isp)s (%(ip)s)...' % config['client'])
	dp.update(6, '[COLOR aqua]Testing From %(isp)s (%(ip)s)...[/COLOR]' % config['client'])

	print_('Selecting best server based on latency...')
	#dp.update(8, "", "", "[COLOR red]Selecting best server based on latency...[/COLOR]")
	best = getBestServer(servers)

	print_(('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
			'%(latency)s ms' % best).encode('utf-8', 'ignore'))
	
	#dp.update(10, "", ('[COLOR red]Server location: %(name)s [%(d)0.2f km]: [/COLOR]'
	#		'[COLOR red]%(latency)s ms[/COLOR]' % best).encode('utf-8', 'ignore'))	
	...
	dp.update(15, "", "", "[COLOR aqua]Testing Download Speed...[/COLOR]")
	...
	dp.update(65, "", "", "[COLOR aqua]Testing Upload Speed...[/COLOR]")
	...
	while ulspeed < 1:
		...
		dp.update(65, "", "", "[COLOR aqua]Testing Upload Speed... [Attempt [/COLOR]" + str(i) + "]")
		ulspeed = uploadSpeed(best['url'], sizes, simple)
		print_()
		print_('Upload: %0.2f M%s/s' %
		   ((ulspeed / 1000 / 1000) * units[1], units[0]))
	...
	dp.update(95, "", "", "[COLOR yellow]Getting Results...[/COLOR]")
